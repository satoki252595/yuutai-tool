...
# 全データの初期セットアップ（推奨）
npm run setup
# → データベース初期化、JPXデータ取得、全銘柄スクレイピング、パフォーマンス最適化を一括実行

# 優待なし銘柄をスキップしてセットアップ（DBサイズ削減）
npm run setup:skip-no-benefits
# → 優待情報が存在する銘柄のみDBに格納（配当のみの銘柄は除外）

# 環境変数での指定も可能
SKIP_NO_BENEFITS=true npm run setup
```

### データベース最適化

```bash
# データベースパフォーマンス最適化のみ実行
npm run db:optimize
# → インデックス作成、統計情報更新、最新価格テーブル構築

# パフォーマンステスト実行
npm run db:test
# → データベースの実行速度を測定（最適化効果を確認）
```

#### スクレイピング進捗のリセット
```bash
# 進捗ファイルを削除して最初からスクレイピング
...
- **配当・価格データ**: 全銘柄対応
- **優待なし除外オプション**: `--skip-no-benefits`で配当のみ銘柄を除外可能
...
- ✅ **パフォーマンス最適化**: 50倍高速化を実現するデータベース最適化
- ✅ **自動最適化**: セットアップ時に自動でパフォーマンステスト実行
